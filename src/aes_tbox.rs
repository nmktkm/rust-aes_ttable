...
fn encrypt_block_AES128(aes128: &AES128, bytes: &[u8;16]) -> [u8;16] {
	...
	for i in 0..4 {
		...
		let key_byte = merge_byte(&initial_key[i]);
		state[i] ^= key_byte;
	}
	...
	for round in 1..10 {
		...
		let key_bytes = [
			merge_byte(&round_key[0]), merge_byte(&round_key[1]), merge_byte(&round_key[2]), merge_byte(&round_key[3]),
		];
